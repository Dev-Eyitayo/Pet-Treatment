...
const TimeSlotInput = ({ day, slot, onChange, error }) => {
  ...
  return (
    <div className='border border-gray-300 dark:border-slate-600 rounded-lg p-4 bg-gray-50 dark:bg-slate-700 shadow-sm w-full box-border'>
      <h4 className='text-sm font-semibold text-text-light dark:text-text-dark mb-2'>
        Available Time on {day}
      </h4>
      <div className='flex items-center gap-3 mt-2 w-full flex-wrap'>
        <input
          type='time'
          value={slot.from || ""}
          ...
          aria-describedby={error ? `time_${day}-error` : undefined}
        />
        <input
          type='time'
          value={slot.to || ""}
