...
//       setErrors((prev) => ({
//         ...prev,
//         certificates: `Invalid files: ${invalidNames}. Only PDF, JPG, and PNG files under 5MB are allowed.`,
//       }));
//       return;
//     }
...
//     }));
//     setCertificateFormData((prev) => prev.filter((_, i) => i !== index));
//   };
...
//     // Try both possible keys to match Postman
//     formData.certificates.forEach((file, index) => {
//       formDataToSend.append(`certificates`, file); // Primary key to match Postman
//       // Alternatively, try certificate_files if certificates fails
//       // formDataToSend.append(`certificate_files`, file);
//     });
...
//           error.response?.data.certificate_files?.[0] ||
...
export default function DoctorApplication() {
  ...
  const [formData, setFormData] = useState({
    bio: "",
    specialization: "",
    certificate_urls: [], // Store URLs instead of files
  });
  const [errors, setErrors] = useState({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [certificatePreviews, setCertificatePreviews] = useState([]);
  ...
  useEffect(() => {
    ...
    //   (error, result) => {
    //     if (error) {
    //       toast.error("Upload failed: " + error.message, { position: "bottom-right" });
    //       return;
    //     }
    ...
    //       setFormData((prev) => ({
    //         ...prev,
    //         certificate_urls: [...prev.certificate_urls, url],
    //       }));
    //       setCertificatePreviews((prev) => [
    //         ...prev,
    //         result.info.resource_type === "image" ? url : null,
    //       ]);
    //     }
    //   }
    // );
  ...
  const validate = () => {
    ...
    if (formData.certificate_urls.length === 0)
      errs.certificate_urls = "At least one certificate is required";
    return errs;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      certificate_urls: formData.certificate_urls, // Send URLs as a list
    };

    console.log("Submitting payload:", payload);
    console.log("API URL:", `${import.meta.env.VITE_API_BASE_URL}/api/applications/`);
    ...
    try {
      ...
      setFormData({ bio: "", specialization: "", certificate_urls: [] });
      setCertificatePreviews([]);
    } catch (error) {
      console.error("Error submitting application:", {
        status: error.response?.status,
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        errorMessage =
          error.response?.data.certificate_urls?.[0] ||
          error.response?.data.bio?.[0] ||
          error.response?.data.specialization?.[0] ||
          error.response?.data.detail ||
          error.response?.data.non_field_errors?.[0] ||
  ...
  const getFileType = (url) => {
    ...
    return extension === "JPEG" || extension === "JPG" || extension === "PNG" ? "Image" : "PDF";
  };
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., Pediatrician"
            className={`w-full rounded-lg border bg-white dark:bg-slate-800 text-text-light dark:text-text-dark px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 ${
              errors.specialization ? "border-red-500" : "border-gray-300 dark:border-slate-600"
            }`}
          />
          {errors.specialization && (
            <p className="text-sm text-red-500 mt-1">{errors.specialization}</p>
        ...
        <div>
          ...
          <button
            ...
            className="w-full rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-slate-800 dark:border-slate-700 dark:text-white p-2 text-left"
          >
            Upload Certificates
          </button>
          {errors.certificate_urls && (
            <p className="text-sm text-red-500 mt-1">{errors.certificate_urls}</p>
          )}
              ...
              {formData.certificate_urls.map((url, index) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500">{getFileType(url)}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => removeCertificate(index)}
