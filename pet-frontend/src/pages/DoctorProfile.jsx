...
const allDoctors = [
  ...
  },
  {
    ...
    availableDays: ["Tuesday", "Thursday"],
    availableTimes: {
      Tuesday: [{ from: "09:00", to: "15:00" }],
      Thursday: [{ from: "10:00", to: "16:00" }],
    },
    bio: "Dr. John Okafor focuses on optimizing pet health through tailored nutrition plans, with 5 years of expertise.",
  },
  // ... (other doctors from your provided DoctorList, with added availableDays and availableTimes)
];

// Generate time slots for a specific date based on available times
const generateAvailableSlots = (selectedDate, availableDays, availableTimes) => {
  const slots = [];
  const dayName = selectedDate.toLocaleString("en-US", { weekday: "long" });

  if (availableDays.includes(dayName)) {
    const timeRanges = availableTimes[dayName] || [];
    timeRanges.forEach((range, index) => {
      let currentTime = new Date(`1970-01-01T${range.from}:00`);
      const endTime = new Date(`1970-01-01T${range.to}:00`);
      // Generate 1-hour slots within the range
      while (currentTime < endTime) {
        slots.push({
          id: index + 1 + slots.length,
...
export default function DoctorProfile() {
  const { doctorId } = useParams();
  const { user, profileData } = useOutletContext(); // Get user and profileData from MainLayout
  const [selectedDate, setSelectedDate] = useState(null);
  const [selectedSlot, setSelectedSlot] = useState(null);
  const [showConfirmation, setShowConfirmation] = useState(false);

  // Determine if viewing the logged-in doctor's profile or another doctor's profile
  const isOwnProfile = user?.role === "doctor" && doctorId === "self";
  const doctor = isOwnProfile
    ? {
        ...
        availableDays: profileData.available_days,
        availableTimes: profileData.available_times,
        bio: profileData.bio,
      }
    : allDoctors.find((d) => d.id === parseInt(doctorId));

  // Generate available slots for the selected date
  const availableSlots = selectedDate && doctor ? generateAvailableSlots(selectedDate, doctor.availableDays, doctor.availableTimes) : [];
  ...
  if (!doctor) {
    return (
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <p className="text-lg text-gray-900 dark:text-white">Doctor not found.</p>
        <Link
          to="/"
          className="mt-4 inline-block bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg shadow-sm transition focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800"
        >
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <section>
        ...
        <div className="mb-6">
          <DatePicker
            ...
            filterDate={isDateAvailable}
            minDate={new Date("2025-06-04")}
            maxDate={new Date(new Date("2025-06-04").setDate(new Date("2025-06-04").getDate() + 30))}
            inline
            calendarClassName="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4"
            dayClassName={(date) =>
              isDateAvailable(date)
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              {availableSlots.map((slot) => (
                <div
                  key={slot.id}
                  className={`flex items-center justify-between bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 sm:p-5 hover:shadow-md transition border-2 ${
                    selectedSlot?.id === slot.id ? "border-blue-600" : "border-transparent"
                  }`}
                  onClick={() => setSelectedSlot(slot)}
                >
                  <div className="flex items-center gap-4">
      ...
      {showConfirmation && (
        <div className="fixed bottom-4 right-4 bg-green-500 text-white p-4 rounded-lg shadow-lg transition-opacity duration-300">
          Your booking request has been submitted! You will be notified when {doctor.name} approves your appointment.
        </div>
      )}
    </div>
  );
